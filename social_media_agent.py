...
class XPlatform(SocialMediaPlatform):
    ...
    def __init__(self, credentials: XCredentials):
        ...
        self.oauth2_client_id = os.environ.get("X_CLIENT_ID")
        self.oauth2_client_secret = os.environ.get("X_CLIENT_SECRET")
    ...
    def connect(self):
        ...
        # Set up OAuth 2.0 client credentials if available
        if self.oauth2_client_id and self.oauth2_client_secret:
            print(f"✅ OAuth 2.0 client credentials found for X API")
            # We'll use these for specific API calls that require OAuth 2.0
    
    def post_with_v2_api(self, text: str, media_url: str = None) -> PostResult:
        """Post content to X using the v2 API.
        ...
        Returns:
            PostResult with status and message
        """
        try:
            ...
            if self.oauth2_client_id and self.oauth2_client_secret:
                try:
                    print("Attempting to post with OAuth 2.0 credentials")
                    # For OAuth 2.0 we need to use requests directly
                    import requests
                    
                    # Get OAuth 2.0 access token
                    token_url = "https://api.twitter.com/2/oauth2/token"
                    
                    # Create a tuple for HTTP Basic Auth (client_id, client_secret)
                    auth = (self.oauth2_client_id, self.oauth2_client_secret)
                    
                    # Make sure both are properly set
                    print(f"OAuth 2.0 Client ID: {self.oauth2_client_id[:5]}...")
                    print(f"OAuth 2.0 Client Secret: {self.oauth2_client_secret[:5]}...")
                    
                    # Use client_credentials grant type for app-only authentication
                    data = {
                    ...
                    # Make the token request (without auth parameter)
                    token_response = requests.post(token_url, data=data)
                    
                    if token_response.status_code != 200:
                        print(f"Failed to get OAuth 2.0 token: {token_response.text}")
                        raise Exception("OAuth 2.0 token request failed")
                    ...
                    payload = {'text': text}
                    if media_url:
                        payload['media'] = {'media_ids': [self._upload_media(media_url)]}
                    ...
                    if response.status_code == 201:
                        ...
                        return PostResult(
                            success=True,
                            message=f"Successfully posted to X with OAuth 2.0. Tweet ID: {tweet_id}",
                            post_url=f"https://twitter.com/i/web/status/{tweet_id}"
                        )
                    else:
                        print(f"OAuth 2.0 posting failed: {response.text}")
                        # Fall back to OAuth 1.0a
                        print("Falling back to OAuth 1.0a")
                except Exception as e:
                    print(f"OAuth 2.0 attempt failed: {str(e)}")
                    print("Falling back to OAuth 1.0a")
            ...
            return PostResult(
                success=False,
                message="Failed to post to X: Unknown error"
            )
        except Exception as e:
            error_message = str(e)
            if "403" in error_message:
                return PostResult(
                    success=False,
                    message="Failed to post to X: Permission error. Your X API key does not have the required access level. Please check the instructions in the app for upgrading your X API access."
                )
            return PostResult(
                success=False,
                message=f"Failed to post to X: {error_message}"
            )
    ...
    def _upload_media(self, media_url: str) -> str:
        ...
        Returns:
            Media ID as string or None if failed
        """
        try:
            print(f"Uploading media from URL: {media_url}")
            
            # For remote files, we need to download first
            if media_url.startswith(('http://', 'https://')):
                ...
                with tempfile.NamedTemporaryFile(suffix=file_ext, delete=False) as temp_file:
                    try:
                        print(f"Downloading media file to {temp_file.name}")
                ...
                try:
                    ...
                    print(f"Uploading media file to X API")
                    media = self.api.media_upload(filename=temp_file_path)
                    media_id = media.media_id_string
                    print(f"Media uploaded successfully. Media ID: {media_id}")
                    return media_id
                except Exception as e:
                    print(f"Error uploading media to X API: {str(e)}")
                    return None
                finally:
                ...
                # Local file
                print(f"Uploading local file to X API")
                media = self.api.media_upload(filename=media_url)
                media_id = media.media_id_string
                print(f"Media uploaded successfully. Media ID: {media_id}")
                return media_id
        except Exception as e:
            print(f"Error in _upload_media: {str(e)}")
            return None
...
class SocialMediaAgent:
    ...
    def post_video_to_x(self, text: str, video_url: str, hashtags: list = None) -> PostResult:
        """Post a video to X (Twitter).
        
        Args:
            text: Text content for the post
            video_url: URL to the video to post
            hashtags: Optional list of hashtags to include
            
        Returns:
            PostResult with success status and message
        """
        if not self._check_platform_available("x"):
            return PostResult(
                success=False,
                message="X posting is not available. Please check your credentials."
            )
        ...
        try:
            ...
            if hashtags and isinstance(hashtags, list):
                ...
                formatted_hashtags = " ".join([f"#{h}" if not h.startswith('#') else h for h in hashtags])
            ...
            print(f"Posting to X with text: {full_text}")
            print(f"Video URL: {video_url}")
            
            # Use the X platform instance to post
            x_platform = self.platforms["x"]
            result = x_platform.post_with_v2_api(full_text, video_url)
            
            if result.success:
                print(f"Successfully posted to X: {result.message}")
                return result
            else:
                print(f"Failed to post to X: {result.message}")
                return result
                
        except Exception as e:
            error_message = str(e)
            print(f"Error posting video to X: {error_message}")
            return PostResult(
                success=False,
                message=f"Failed to post video to X: {error_message}"
            )
    
    def add_platform(self, platform_name: str, credentials):
        """Add a social media platform to the agent.
        
        Args:
            platform_name: Name of the platform to add
            credentials: Credentials for the platform
            
        Returns:
            None
        """
        if platform_name.lower() == "x" or platform_name.lower() == "twitter":
            self.platforms["x"] = XPlatform(credentials)
        elif platform_name.lower() == "facebook":
            self.platforms["facebook"] = FacebookPlatform(credentials)
        elif platform_name.lower() == "instagram":
            self.platforms["instagram"] = InstagramPlatform(credentials)
        elif platform_name.lower() == "linkedin":
            self.platforms["linkedin"] = LinkedInPlatform(credentials)
        else:
            print(f"Unsupported platform: {platform_name}")
            
    def _check_platform_available(self, platform_name: str) -> bool:
        ...
        """
        return platform_name.lower() in self.platforms 

    def post_to_platform(self, platform_name: str, text: str, media_url: str = None) -> PostResult:
        ...
        Returns:
            PostResult with status and message
        """
        if not self._check_platform_available(platform_name):
            return PostResult(
                success=False,
                message=f"{platform_name} posting is not available. Please check your credentials."
            )
        ...
        try:
            ...
            if media_url:
                print(f"Posting to {platform_name} with media: {media_url}")
                if platform_name.lower() == "x":
                    return platform.post_with_v2_api(text, media_url)
                else:
                    # Generic implementation for other platforms
                    # This would need to be customized for each platform
                    return PostResult(
                        success=False,
                        message=f"Media posting not implemented for {platform_name}"
                    )
            else:
                print(f"Posting to {platform_name}: {text}")
                if platform_name.lower() == "x":
                    return platform.post_with_v2_api(text)
                else:
                    # Generic implementation for other platforms
                    return PostResult(
                        success=False,
                        message=f"Posting not implemented for {platform_name}"
                    )
        except Exception as e:
            error_message = str(e)
            print(f"Error posting to {platform_name}: {error_message}")
            return PostResult(
                success=False,
                message=f"Failed to post to {platform_name}: {error_message}"
            ) 