...
class DatabaseAgent:
    ...
    def search_articles(self, query: SearchQuery) -> List[SearchResult]:
        try:
            # Use similarity search without filters if none are provided
            if query.filters:
                ...
                SearchResult(
                    article=NewsArticle(
                        ...
                        content=ArticleContent(
                            ...
                            markdown=doc.metadata.get('markdown', ''),
                            url=doc.metadata.get('url', ''),
                            source=doc.metadata.get('source', ''),
                            published_date=doc.metadata.get('published_date', datetime.now().isoformat())
                        ),
                        link=doc.metadata.get('url', ''),
                        source=doc.metadata.get('source', ''),
                        source_type=doc.metadata.get('source_type', 'web'),
                        published_date=datetime.fromisoformat(doc.metadata.get('published_date', datetime.now().isoformat())) if doc.metadata.get('published_date') else None,
